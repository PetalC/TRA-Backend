...
const getLimitListReport = async ({
  ...
  try {
    ...
    if (
      ...
      reportColumn.includes('approvalType') ||
    ...
    response.forEach((limit) => {
      ...
      if (
        limit.activeApplicationId &&
        limit.activeApplicationId[0] &&
        limit.activeApplicationId[0].approvalType
      ) {
        console.log(limit.activeApplicationId[0].approvalType)
        limit.approvalType = StaticData.approvalType
        .find((i) => i._id === limit.activeApplicationId[0].approvalType )?.name ?? '-';
        console.log(limit.approvalType)
