...
const getApplicationList = async ({
  ...
  try {
    ...
    if (requestedQuery.status) {
      if (requestedQuery.status.includes("APPROVED") && requestedQuery.approvalType) {
        let array = requestedQuery.status.split(",");
        
        let index = array.indexOf("APPROVED");
        if (index > -1) {
        ...
        queryFilter = {
          "$or": [
              {"status": {"$in": array}},
              {
                "$and" : [
                  {"status": {"$in" : ['APPROVED']}},
                  {"approvalType": {"$in": [requestedQuery.approvalType]}}
                ]
              }
          ]
        } 
        
      } else queryFilter.status = { $in: requestedQuery.status.split(',') };
      if (isForDownload) {
        filterArray.push({
...
const storeCreditLimitDetails = async ({ requestBody }) => {
  try {
    ...
    await Application.updateMany(
      ...
      { $set: { clientReference: requestBody?.clientReference } }
    );
    application = await Application.findById(requestBody.applicationId)
      .select('_id applicationStage')
      .lean();
...
const generateNewApplication = async ({
  ...
  try {
    ...
    if (application) {
      ...
      if (clientReference) {
        applicationDetails.clientReference = clientReference;
      }
