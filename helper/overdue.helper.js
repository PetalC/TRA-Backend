...
const getOverdueList = async ({
  ...
  try {
    const queryFilter = {};
    if(requestedQuery?.csvId != undefined && requestedQuery?.csvId != -1) {
      queryFilter.bulkUploadId = mongoose.Types.ObjectId(requestedQuery.csvId);
    }
    requestedQuery.page = requestedQuery.page || 1;
    requestedQuery.limit = requestedQuery.limit || 5;
    requestedQuery.sortOption = requestedQuery.sortOption || 0;
    if (!isForRisk) {
    ...
    if (requestedQuery.sortOption == 2) {
      query.push({
        $sort: {
          '_id.year': -1,
          '_id.month': -1,
    ...
    if (requestedQuery.sortOption == 3) {
      query.push({
        $sort: {
          '_id.year': 1,
          '_id.month': 1,
    ...
    if (requestedQuery.sortOption == 0) {
      query.push({
        $sort: { client: 1 },
      });
    }
    if (requestedQuery.sortOption == 1) {
      query.push({
        $sort: { client: -1 },
      });
    }
    ...
    const overdueList = await Overdue.aggregate(query)
      .allowDiskUse(true)
...
const addOverdue = async ({
  ...
  createdByType,
  createdById,
  isCreatedByBulkUpload = false,
  bulkUploadId = null,
}) => {
  try {
    const overdue = await Overdue.create({
      clientId,
      debtorId,
      acn,
      dateOfInvoice,
