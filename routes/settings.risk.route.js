...
router.get('/document-type', async function (req, res) {
  try {
    ...
    option.select = '_id documentFor documentTitle role updatedAt';
    ...
    if (
      ...
      documentTypes.docs.forEach((document) => {
        const areEqual = document.role?.includes('client') && document?.role?.includes('risk');
        let role;
        if(document.role && document.role[0]) {
          role = transformWord(document.role[0]);
        }
        document.role = areEqual ? ['Both'] : role;
...
router.post('/document-type', async function (req, res) {
  ...
  try {
    let document = await DocumentType.findOne({
      isDeleted: false,
      documentFor: req.body.documentFor.value,
      documentTitle: req.body.documentTitle,
      role: req.body.roleFor,
    }).lean();
    if (document) {
      return res.status(400).send({
        status: 'ERROR',
