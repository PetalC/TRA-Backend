 ...
 */
router.get('/download-form', async function (req, res) {
  try {
    const response = await StaticFileHelper.downloadDocument({
      filePath: 'static-files/application-dump/Credit_Limit_Application.docx',
    });
    res.header(
      'Content-Type',
      'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
    );
    res.setHeader('Content-Disposition', 'attachment; filename=' + "Credit Limit Application.docx");
  ...
  try {
    ...
    if (req.body.update === 'field') {
        ...
        await Application.updateMany(
          {
            $and: [
              ...
              { debtorId: currentApplication.debtorId }
            ]
          },
          { $set: { clientReference: req.body.clientReference } }
        );
      }
      ...
      if (req.body.approvalType) {
        applicationUpdate.approvalType = req.body.approvalType;
      }
    ...
    console.log("applicationUpdate: ", applicationUpdate)
     //TODO notify user
    await Application.updateOne(
      { _id: req.params.applicationId },
      applicationUpdate,
