...
router.post('/upload-csv', upload.single('overdue-csv'), async (req, res) => {
  try {
    const user = await ClientUser.findOne(req.user._id).lean();
    ...
    csv
      ...
      .on('end', async function () {
        ...
        for (const overdue of overdues) {
          ...
          const createdByType = 'client-user';
...
router.get('/download/sample-csv/', async function (req, res) {
  try {
    const filePath = path.join(__dirname, '..', 'static-files', 'Bulk-Upload-Overdues-Sample.csv');
    fs.readFile(filePath, (err, data) => {
      ...
      res.setHeader('Content-Disposition', 'attachment; filename=BulkUploadOverdueSample.csv');
      res.send(data);
    });
  } catch (e) {
    Logger.log.error('Error occurred in downloading Sample CSV: ', e.message || e);
    res.status(500).send({
      status: 'ERROR',
      message: e.message || 'Something went wrong, please try again later.',
    });
